...
//        window.setStatusBarColor(Color.TRANSPARENT);
     ...
     * @return
     */
    public static int getTitleAndStatusHeight(Context context){
        int size= (int) (context.getResources().getDimension(R.dimen.title_dp));
        if (Build.VERSION.SDK_INT >= MIN_API) {
            size+=StatusBarUtil.getStatusBarHeight(context);
        }
       return size;
    }
    /** 获取状态栏高度 */
